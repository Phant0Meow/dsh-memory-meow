...
- **Cache-friendly by design**: the static `meow-memory:guide` section (order 130, right
  ...
  text, KV-cache friendly. The dynamic snapshot (`soul`/`user` in full, plus a memory index
...
npm run test           # 86 logic tests: db / bm25 / migrate / inject / reflect / dream / tools
```
