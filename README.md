...
[![中文](https://img.shields.io/badge/中文-简体中文-brightgreen)](README.md)
[![README](https://img.shields.io/badge/README-说明文档-blue)](README.md)
...
- **缓存友好设计**：静态 `meow-memory:guide` section（order 130）在 system prompt 中注册一次——
  文本恒定，KV 缓存友好。动态快照（`soul`/`user` 全量 + 记忆索引 + 关键词命中的短 fact/lesson）
  作为第一条用户消息的前缀注入；system prompt 本身不随会话变化，快照在整场会话中冻结。
  已见记忆（`injected` + `searched`）按会话记录，绝不重复注入或重复检索。
...
- **按窗口 dream**：夜间（按 `timeZone` 计算，默认 00:00–07:00，空闲时）每个最后发言
  ...
  会话上下文。无 live agent 且超过 24h 的旧窗口不处理。
