...
- **Cache-friendly by design**: memory is injected as a plain user-message suffix on the
  session's first user message. The system prompt never changes, and the snapshot is frozen
  for the whole session — later memory updates cannot invalidate the request prefix.
- **Human-owned memory**: `.dsh-meow/PROJECT.md` per workspace. Plain Markdown, human-editable,
  git-diffable, deletable with `rm`.
